...
def generate_mermaid_tree(simulation: dict) -> str:
    ...
    selected_node_ids = {
        node["id"] for turn_num in all_turns for node in nodes_by_turn.get(turn_num, [])
    }
    lines.append("")
    ...
    lines.append(
        f"    classDef focused fill:{focused_fill},stroke:{focused_stroke},stroke-width:3px"
    )
    lines.append(
        f"    classDef default fill:{default_fill},stroke:{default_stroke},stroke-width:1px"
    )
...
def generate_simplified_tree(simulation: dict, max_nodes: int = 30) -> str:
    ...
    for turn_num in sorted(turn_nodes.keys()):
        ...
        sorted_nodes = sorted(
            nodes, key=lambda n: node_connections[n["id"]], reverse=True
        )
    ...
    filtered_edges = [
        e
        for e in simulation["graph"]["edges"]
        if e["source_node_id"] in selected_nodes
        and e["target_node_id"] in selected_nodes
    ]
    ...
    simplified = {
        "graph": {
            "nodes": [
                n for n in simulation["graph"]["nodes"] if n["id"] in selected_nodes
            ],
...
def main():
    ...
    if args.simplified:
        ...
        print(
            f"// Simplified tree with ~{min(args.max_nodes, len(simulation['graph']['nodes']))} nodes",
            file=sys.stderr,
        )
    else:
        mermaid = generate_mermaid_tree(simulation)
        print(
            f"// Full tree with {len(simulation['graph']['nodes'])} nodes",
            file=sys.stderr,
        )
