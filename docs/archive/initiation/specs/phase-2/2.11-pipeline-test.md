# Spec 2.11: Pipeline Integration Test

## Objective
Create integration tests that verify the complete turn processing pipeline works end-to-end.

## Context
- Reference: IMPLEMENTATION_PLAN.md Phase 2 deliverable
- Tests the full flow: API → SessionService → Extraction → Graph → Question
- Uses mocked LLM responses to avoid actual API calls
- Validates data flows correctly through all layers

## Input Files
- All Phase 2 specs (2.1-2.10)
- `PRD.md` - Expected behavior
- `tests/conftest.py` - Shared fixtures from Phase 1
...
## Requirements
1. Test complete flow: create → start → turn(s) → verify
2. Use mocked LLM responses (no actual API calls)
3. Verify extraction results in response
4. Verify graph state updates across turns
5. Verify data persisted to SQLite
6. Test error handling (LLM errors, invalid session)
7. Verify response format matches PRD Section 8.6
...
# Run with coverage
pytest tests/integration/test_session_flow.py -v --cov=src

# Run all Phase 2 tests
pytest tests/ -v -k "test_models or test_llm or test_extraction or test_graph or test_question or test_session or test_turn or test_pipeline"
```

## Success Criteria
- [ ] Complete session flow test (create → start → turn → verify)
- [ ] Multiple turns accumulate in graph
- [ ] Non-extractable responses handled gracefully
- [ ] Nodes persisted to SQLite
- [ ] Utterances persisted for provenance
- [ ] LLM errors handled gracefully
- [ ] Invalid session returns 404
- [ ] Response format matches PRD Section 8.6
