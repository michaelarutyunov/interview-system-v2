# Spec 3.8: Strategy Selector

## Objective
Create the strategy selector service for adaptive strategy selection.

## Context
- Reference: PRD Section 5.3 (Strategy Selection), IMPLEMENTATION_PLAN.md Section 3.8
- v1 Reference: `/home/mikhailarutyunov/projects/graph-enabled-ai-interviewer/src/modules/strategy_selector.py`
- Orchestrates: applicability filtering → focus generation → arbitration → selection
- Returns selected strategy with full scoring provenance

## Input Files
- `src/services/scoring/arbitration.py` - ArbitrationEngine from spec 3.7
- All 5 scorers from specs 3.2-3.6
- v1: `/home/mikhailarutyunov/projects/graph-enabled-ai-interviewer/src/modules/strategy_selector.py`
...
class StrategyService:
    ...
    async def select(
        ...
        if not candidates:
            ...
            return self._get_fallback_result(graph_state, recent_nodes)
...
## Requirements
1. Defines STRATEGIES constant with 3 strategies (deepen, broaden, cover_element)
2. Generates focuses per strategy type
3. Uses ArbitrationEngine to score all candidates
4. Selects highest-scoring candidate
5. Returns alternatives list
6. Handles fallback when no candidates
...
# Verify import
python3 -c "from src.services.strategy_service import StrategyService; print('OK')"
```

## Success Criteria
- [ ] `StrategyService` class created with `select()` method
- [ ] `STRATEGIES` constant with 3 strategies
- [ ] `_get_possible_focuses()` generates focuses per strategy type
- [ ] Scores all candidates via ArbitrationEngine
- [ ] Returns `SelectionResult` with selected strategy and alternatives
- [ ] Handles fallback when no candidates available
- [ ] All tests pass
