...
| **graph.canonical_edge_density** | How interconnected deduplicated topics are | Edge-to-concept ratio in the canonical graph: `canonical_edge_count / canonical_concept_count`. Higher = more relationships between stable high-level concepts. Returns 0.0 if no canonical concepts exist yet | Low (<0.5) = isolated topics with few links; High (>1.0) = well-connected structure where topics relate to each other |
