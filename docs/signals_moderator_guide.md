...
| **graph.node.exhaustion_score** | 0.0-1.0 score of exploration depth | Weighted sum of three factors: (1) `turns_since_last_yield / 10 × 0.4` + (2) `focus_streak / 5 × 0.3` + (3) `shallow_response_ratio × 0.3`, where `shallow_response_ratio` counts responses with depth category `"surface"` OR `"shallow"` | Higher (0.7+) = thoroughly explored; Lower (0.0-0.3) = fresh territory |
