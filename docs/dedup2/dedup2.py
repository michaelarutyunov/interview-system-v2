...
SLOT_MERGE_THRESHOLD = (
    0.88  # Higher threshold = less merging = more granular, focused slots
)
...
def split_into_unequal_chunks(
    nodes: List[dict], n_turns: int = 10, seed: int = 42
) -> List[List[dict]]:
    """
...
def propose_slots_with_kimi(
    surface_nodes: List[dict], existing_slot_names: List[str], api_key: str
) -> List[dict]:
    """
    ...
    nodes_text = "\n".join([f"- {n['id']}: {n['label']}" for n in surface_nodes])
  ...
  {{
    ...
    try:
        ...
        for slot in proposed_slots:
            if not all(
                k in slot
                for k in ["slot_name", "description", "supporting_surface_ids"]
            ):
...
class SlotDiscoverySystem:
    ...
    def get_existing_slot_names(self) -> List[str]:
        ...
        return [
            h.slot_name for h in self.slot_hypotheses.values() if h.status == "active"
        ]
    ...
    def process_turn(
        self, surface_chunk: List[dict], turn_id: int, api_key: str
    ) -> dict:
        """
        ...
        for slot in proposed_slots:
            print(
                f"  - {slot['slot_name']}: {len(slot['supporting_surface_ids'])} nodes"
            )
        ...
        for proposed in proposed_slots:
            ...
            if self.slot_hypotheses[slot_id].status == "candidate":
                ...
                results["merged_count"] += 1
                print(
                    f"  ~ Merged into active: {proposed['slot_name']} -> {slot_id[:8]}"
                )
...
def main():
    ...
    sorted_canonical = sorted(
        system.canonical_slots.values(), key=lambda s: len(s.surface_ids), reverse=True
    )
    ...
    for slot_name in sorted(
        slot_to_surfaces.keys(), key=lambda x: (slot_to_status[x] != "canonical", x)
    ):
