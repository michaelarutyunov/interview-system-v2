...
| `edge_extraction` | dict\|null | Stage 4.5B diagnostics per turn | — |

### `turns[].edge_extraction` (added May 2026)

Present when `enable_edge_extraction_stage=true`. Null when flag is OFF.

| Field | Type | Description |
|-------|------|-------------|
| `confirmed_count` | int | Number of confirmed edges this turn |
| `rejected_count` | int | Number of rejected candidate pairs |
| `low_confidence_count` | int | Confirmed edges with `confidence="low"` |
| `confirmed_edges[]` | list | Per-edge: `source_node_id`, `target_node_id`, `edge_type`, `confidence`, `utterance_id` |
| `rejected_candidates[]` | list | Per-rejection: `source_node_id`, `target_node_id`, `rejection_reason` |
