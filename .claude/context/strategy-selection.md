...
**Output:** `StrategySelectionOutput` with `strategy`, `focus`, `signals`, `node_signals`, `score_decomposition`.
...
- Initializes `node_signals` dict from `node_tracker.get_all_states().keys()` — surface UUIDs only (post-vxz6 keyspace)
