...
**Invariant: GraphUpdate + EdgeExtractionBridge run strictly before StrategySelection within the same turn.** `record_yield` was moved from GraphUpdateStage to EdgeExtractionBridgeStage (D4/B7) so that yield credit reflects both Stage 4 node writes AND Stage 4.5B edge writes. It fires unconditionally — the `is_empty()` guard was removed (p4t3) because it silently skipped yields when `edges_added=0`.

**Consequence**: any state mutation made in `record_yield()` (Stage 4.6) is visible to signal detectors in StrategySelectionStage (Stage 6). Therefore, **resetting any signal-relevant counter in `record_yield()` causes the signal to read as zero at detection time**, even though it was correct the previous turn. This is the canonical "focus_streak bug" — do not reintroduce it.
...
- Resets to `0` in `record_yield()` (Stage 4.6, EdgeExtractionBridgeStage) for the yielding node.
- Inside `update_focus()` (Stage 6), **after** focus accounting, the loop ticks `s.turns_since_last_yield += 1` for **every node in `self.states.items()`** — not just the focused one. Without the global tick, exhaustion scores never grow on unfocused nodes and rotation breaks.
- `record_yield` does NOT gate on `graph_changes.is_empty()` — the guard was removed (p4t3) because after the B7 move to the bridge stage, `nodes_added=0` and `edges_added=0` (when flag OFF) caused every yield to be silently skipped, triggering premature `all_nodes_exhausted`.
...
4. **Never reset signal-relevant state in `record_yield()` (Stage 4.6, EdgeExtractionBridgeStage).** Stage 4.6 runs before Stage 6 (signal detection) within the same turn — any reset there is invisible-to-impossible to recover.
