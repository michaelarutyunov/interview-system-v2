...
- **Chain topology signals hardcode `leads_to`:** `ChainTopologySignalDetector` (`src/signals/graph/chain_topology_signals.py:108`) filters edges to `edge_type == "leads_to"`. MEC uses `leads_to` edges, but JTBD uses `triggers`/`enables`/`supports`/`addresses`, CIT uses its own edge types, etc. For non-MEC methodologies, the adjacency list is always empty, producing vacuously-true `gap.above` for all nodes and `gap.below` only for terminal-level nodes. This means `ascend` and `ground` pass their `valid_when` gates on incorrect signals. Fix: read the methodology YAML's ontology edge definitions instead of hardcoding `leads_to`. Chain rules YAML (`config/chain_rules/`) is for reporting only — not for runtime signals. See `.claude/context/signal-detection-graph.md`.
- **Canonical slot nodes always gated on chain topology signals:** `ChainTopologySignalDetector` reads surface nodes from the DB (`kg_nodes` table) and produces chain topology signals keyed by surface node IDs. Canonical slot nodes (`slot_*` IDs) come from `NodeStateTracker` and are included in the scoring loop, but have no `convgraph.node.chain.*` entries. The `valid_when` gate check `node_signal_dict.get(gate_signal)` returns `None` (falsy), so canonical slot nodes are always gated out of node-bound strategies. Surface nodes (UUIDs) work correctly. See `.claude/context/signal-detection-graph.md`.
